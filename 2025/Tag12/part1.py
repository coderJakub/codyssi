...
for instr in instrs.split("\n"):
    if instr.split(" ")[0] == "SHIFT":
        grid = shift(instr.split(" ")[1:])
    else:
        grid = maths(instr.split(" "))

print(f'Part 1: {max(max([sum(line) for line in grid]), max([sum(line) for line in list(zip(*grid))]))}')